...
subroutine initialize
    ...
    open(unit = 1, file = 'baer_nunziato_params.ini', status = 'UNKNOWN')
    ...
    allocate ( Shi(nVar, nDOF(1), nDOF(2), IMAX, JMAX) )
    allocate ( duh(nVar, nDOF(1), nDOF(2), IMAX, JMAX) )
    allocate ( qbnd(nVar, nFac, nDOF(2), IMAX, JMAX) )
    allocate ( Fbnd(nVar, 4, nDOF(2), IMAX, JMAX) )
    allocate ( Fp(nVar, nDOF(2), IMAX+1 , JMAX ) )
    allocate ( Fm(nVar, nDOF(2), IMAX+1 , JMAX ) )
    allocate ( Gp(nVar, nDOF(1), IMAX , JMAX+1 ) )
    allocate ( Gm(nVar, nDOF(2), IMAX+1 , JMAX ) )
