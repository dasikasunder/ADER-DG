...
program ADERDG
    ...
    do while (time < tend)
        ...
        do j = 1,  JMAX
            do i = 1, IMAX
                call ader_space_time_predictor(qhi(:,:,:,i,j), Fhi(:,:,:,:,i,j), &
                    Shi(:,:,:,i,j), qBnd(:,:,:,i,j), FBnd(:,:,:,i,j), uh(:,:,:,i,j))
            end do
        end do
        ...
        do j = 1, JMAX
            do i = 1, IMAX
                call  volume_integral(duh(:,:,:,i,j),Fhi(:,:,:,:,i,j),Shi(:,:,:,i,j))
            end do
        end do
        ...
        do j = 1, JMAX
            do i = 1, IMAX
                call surface_integral(duh(:,:,:,i,j), Fm(:,:,i,j), Fp(:,:,i+1,j), Gm(:,:,i,j), Gp(:,:,i,j+1))
            end do
        end do
